...
for order in list_orders:
    if order["side"] == "sell":
        ...
        print(f"{temp_price} USD  {temp_size_price} USD  (\033[92m +{temp_diff} \033[0m) (\033[92m {temp_percent_diff:.1f} % \033[0m)")
...
for order in list_orders:
    if order["side"] == "buy":
        ...
        temp_diff = round(temp_price - current_price, 2)
        temp_percent_diff = 100 * temp_diff / current_price
        print(f"{temp_price} USD  {temp_size_price} USD  (\033[91m {temp_diff} \033[0m) (\033[91m {temp_percent_diff:.1f} % \033[0m)")
print()